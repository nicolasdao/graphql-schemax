...
describe('Schemax', () => {
	describe('.toString()', () => {
		describe('compilation', () => {			
			...
			it('02 - Should fail when the schema is invalid (e.g., missing type definition)', () => {
				const schema = [
					'type Mutation', {
						invite: { 
							users:[{ 
		...
		describe('merging', () => {	
			...
			it('02 - Should support merging same types by keeping the longest when merging multiple schemax into a single valid GraphQL schema.', () => {
				const expected = compressString(`
				enum Hello {
					Jacky
					Peter
			...
			it('03 - Should support renaming types when merging multiple schemax into a single valid GraphQL schema.', () => {
				const expected = compressString(`
				enum Hello {
					Jacky
					Peter
			...
			it('04 - Should support merging same types by keeping the shortest when merging multiple schemax into a single valid GraphQL schema.', () => {
				const expected = compressString(`
				enum Hello {
					Jacky
					Peter
			...
			it('05 - Should support merging same types with a custom reducer when merging multiple schemax into a single valid GraphQL schema.', () => {
				const expected = compressString(`
				enum Hello {
					Jacky
					Peter
		...
		describe('directive', () => {	
			...
			it('02 - Should support multiple identical directives.', () => {
				const expected = compressString(`
				directive @deprecated(
					reason: String = "No longer supported"
				) on FIELD_DEFINITION | ENUM_VALUE
		...
		describe('constructor', () => {
			it('01 - Should support constructor that accept an array schema definitions.', () => {
				const expected = compressString(`
				enum Hello {
					Jacky
					Peter
		...
		describe('enums', () => {
			it('01 - Should support custom names on enums', () => {
				const expected = compressString(`
				type Query {
			...
			it('02 - Should support required anonymous enums', () => {
				const expected = compressString(`
				type Query {
					products(type: ProductTypeEnum!): Type_12078318863
				}
			...
			it('03 - Should support array of anonymous enums', () => {
				const expected = compressString(`
				type Query {
					products(type: [ProductTypeEnum]!): Type_12078318863
				}
			...
			it('04 - Should fail when invalid enums are defined', () => {
				const schema = [
					'type Mutation', {
						invite: { 
							users:[{ 
		...
		describe('complex objects', () => {
			it('01 - Should support nested complex objects', () => {
				const expected = compressString(`
				type Folder {
					id: ID!
				}
			...
			it('02 - Should support nested complex objects in fields', () => {
				const expected = compressString(`
				type Folder {
					id: ID!
		...
		it('01 - Should support required anonynous array types', () => {
		...
		it('02 - Should support for shortcuts \'!\', \'!0\' and \'#\' to respectively replace \'__required\', \'__noempty\', and \'__name\'', () => {
		...
		it('03 - Should support a single object argument to pass both the schema definition and options.', () => {
			const expected = compressString(`
			enum Hello {
				Jacky
				Peter
